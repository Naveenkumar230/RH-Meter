...
// ============================================================
void setup() {
    // 1. HARDWARE SAFETY: Disable Brownout IMMEDIATELY
    WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0); 

    Serial.begin(115200);
    
    // 2. BOOT DELAY: Allow power to stabilize before starting high-current WiFi
    delay(2000); 
    Serial.println("\n=== Factory Monitor Pro v3.0 ===");

    // 3. WATCHDOG: Initialized AFTER power stabilization
    ...
    // 4. I2C: Fixed at 50kHz for GoldenMorning LCD stability
    Wire.begin(Config::I2C_SDA, Config::I2C_SCL, 50000);

    // 5. LCD INIT: Slower handshake sequence
    lcd.init();
    delay(500); 
    lcd.backlight();
    lcd.clear();
    lcdCreateChars();
    lcdSplash();
    Serial.println("[LCD] Initialised at 50kHz");

    // 6. WIFI: WiFiManager with Auto-Healing
    WiFiManager wm;
    // Set a timeout for the portal so it doesn't stay in AP mode forever
    ...
    if (!wm.autoConnect("FactoryMonitor_Setup", "password123")) {
        Serial.println("[WiFi] Portal Timeout — starting offline mode");
        wifiOnline = false;
    } else {
        wifiOnline = true;
